// packages/database/drizzle/schema/auth.ts
//
// Kernel / identity layer (ARCHITECTURE.md §2). Better Auth-managed tables: user, session,
// account, verification, passkey, organization, member, invitation, twoFactor — plus the
// supastarter additions for billing (purchase) and notifications (notification +
// userNotificationPreference). Column naming follows Better Auth's expected casing
// (camelCase), distinct from the snake_case used in the rest of the Virn schema.
//
// The `notificationTypeEnum` is the single source of notification kinds across the platform
// (BUILD_PLAN.md: reuse Better Auth's notification table, do not add a parallel one). Virn
// domain notification types are appended here so the workflow/run/governance modules can
// emit them without a schema change later.
