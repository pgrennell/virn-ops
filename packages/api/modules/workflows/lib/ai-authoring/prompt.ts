...
function builderContractBlock(): string {
	return [
		...
		"   one namespace).",
		"4. Workflows that schedule a step deadline relative to launch use",
		"   dueType='offset_from_start' WITH dueOffsetDays set to a positive integer. Use",
		"   dueType='none' (or omit) when no deadline is needed. No other dueType values",
		"   are supported right now -- if a user asks for 'X days after step Y', leave",
		"   dueType='none' and put the timing in the step description instead.",
		...
		'      "dueOffsetDays": 7,    // only when dueType="offset_from_start"',
		'      "fields": [',
		"        {",
