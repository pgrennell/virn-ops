...
- **Antigravity is for the FIRST browser verification of a NEW phase only — re-runs are free via Playwright.** Once a flow is verified, codify it as a self-contained headless spec under `apps/saas/tests/` (own magic-link auth + `@virn/database` seeding + screenshot capture to `docs/reviews/<phase>/`, following the existing dated specs). Every *re-run* then goes through `pnpm --filter saas exec playwright test <spec>` (or `pnpm e2e:ci`) at **zero Antigravity credits** — never re-drive an already-spec'd flow in Antigravity. Reserve Antigravity for a phase that has a briefing but no spec yet. Cheaper Antigravity knobs when it is needed: keep Flash, drop reasoning effort High→Low, set overage to "never auto-spend credits".
