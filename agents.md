...
- **Default to a headless Playwright spec for UI verification; avoid Antigravity.** For ANY UI flow whose selectors + expected behavior are deterministic (almost all of it — especially when Claude Code wrote the UI), author a self-contained spec under `apps/saas/tests/` (own magic-link auth + `@virn/database` seeding + screenshot capture to `docs/reviews/<phase>/`, following the existing dated specs) and run it via `pnpm --filter saas exec playwright test <spec>` (or `pnpm e2e:ci`). Do NOT reach for an Antigravity briefing just because a phase is new/unspec'd. Antigravity is a LAST RESORT only for genuinely *exploratory* sessions (an agent reasoning about an unfamiliar screen) and only when the user explicitly opts in. A flow that needs a live runtime to advance (e.g. an Inngest orchestrator stepping through `step.sleep`) is NOT an Antigravity case — unit-test the pure logic + smoke it against a local Inngest Dev Server here.
- **Verify role-gated UI server-side, not via a seeded non-admin browser session.** A drizzle-seeded non-admin `member` cannot be driven through a magic-link login into an *active* better-auth org (the org route 404s even with `onboardingComplete=true`), so skip the browser scenario and assert the gate with an oRPC authz/contract test — every mutating procedure is an `adminOrgProcedure` and pages use `assertCanSee(NAV_AREAS.<area>)`.
